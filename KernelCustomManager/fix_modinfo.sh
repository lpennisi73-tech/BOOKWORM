#!/bin/bash
# Fix pour la détection des modules signés sur Debian 13
# modinfo nécessite root pour lire les signatures

echo "=========================================="
echo "FIX MODINFO - Ajout de la fonction modinfo-check"
echo "=========================================="
echo ""

# Créer le helper mis à jour
cat > /tmp/kernelcustom-helper.new << 'HEREDOC'
#!/bin/bash
#
...
chmod +x /tmp/kernelcustom-helper.new

echo "✅ Nouveau helper créé dans /tmp/kernelcustom-helper.new"
echo ""
echo "Installation..."
echo ""

sudo cp /tmp/kernelcustom-helper.new /usr/local/bin/kernelcustom-helper
sudo chmod 755 /usr/local/bin/kernelcustom-helper

echo "✅ Helper mis à jour!"
echo ""
echo "Test de la nouvelle fonction modinfo-check..."
echo ""

# Tester
TEST_MODULE=$(find /lib/modules/6.17.11-test/kernel/drivers/net/ -name "dummy.ko" -type f | head -1)
if [ -n "$TEST_MODULE" ]; then
    echo "Test sur: $TEST_MODULE"
    pkexec /usr/local/bin/kernelcustom-helper modinfo-check "$TEST_MODULE"
    echo ""
    echo "✅ Test réussi!"
else
    echo "⚠️  Module de test non trouvé, impossible de tester"
fi
